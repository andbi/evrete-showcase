    ...
    public static class EntityKnowledgeType extends TypeWrapper<Entity> {
        ...
        public TypeField getField(String name) {
            ...
            return switch (parts[0]) {
                case "flags" -> declareBooleanField(name, entity -> entity.getFlag(key, false));
                case "integers" -> declareIntField(name, entity -> entity.getNumber(key, 0));
                case "doubles" -> declareDoubleField(name, entity -> entity.getDouble(key, 0.0));
                case "properties" -> declareField(name, Entity.class, entity -> entity.getProperty(key));
                default -> throw new IllegalStateException("Unknown prefix: " + parts[0]);
            };
        }
    }
